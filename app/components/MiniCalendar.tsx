        ...
        return (
            <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
                <Pressable onPressIn={handlePressIn} onPressOut={handlePressOut}>
                    <View className={`${pressed ? "bg-blue-700" : "bg-blue-500"} rounded-full p-1`}>
        ...
        <View className="w-full border-2 border-gray-200 dark:border-gray-500 p-3 rounded-xl">
            ...
            <View className="flex-row justify-between mb-1 mt-2">
                {weekdayShort.map((day, i) => (
                    <Text key={i} className="flex-1 text-center text-md font-bold text-gray-500">
                        {day}
                    </Text>
                ))}
            </View>
            ...
            <View className="flex-row flex-wrap mt-3">
                {daysArray.map((day, index) => {
                  ...
                  `}
                                >
                                    <Text className={`${isStreakDay ? "text-white" : "text-gray-400 text-md font-bold dark:text-gray-500"}`}>{day}</Text>
                                </Pressable>
                            ) : (
