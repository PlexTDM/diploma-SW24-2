...
export default function TabBar({
  ...
  const btnWidth = dimensions.width / (state.routes.length - 1);
  ...
  const animatedStyle = useAnimatedStyle(() => {
    return { transform: [{ translateX: tabPosition.value }] };
  });
  return (
    <View
      className="flex-row absolute bottom-[50px] left-0 right-0 items-center justify-between mx-20 py-4 bg-white dark:bg-slate-950 rounded-full p-2 shadow-lg"
      key={state.key}
    ...
    >
      ...
      {state.routes.map((route, index) => {
        ...
        const onPress = () => {
          tabPosition.value = withSpring((index - 1) * btnWidth, {
            duration: 1000,
            dampingRatio: 0.8,
